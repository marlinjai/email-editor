...
/** Every real migration, in order; synthetic files below use 9xxx so they sort after all of them. */
const SHIPPED = (await readdir(DEFAULT_MIGRATIONS_DIR)).filter((f) => f.endsWith('.sql')).sort();
...
describe('migrations', () => {
  it('builds the schema from an empty database', async () => {
    ...
    expect(result.applied).toEqual(SHIPPED);
    expect(SHIPPED.slice(0, 2)).toEqual(['0001_foundation.sql', '0002_templates_assets.sql']);
  ...
  it('is a no-op the second time (re-entry after completion)', async () => {
    ...
    expect(again.alreadyApplied).toBe(SHIPPED.length);
  });
  ...
  it('leaves nothing half-applied when a file fails, and resumes from it once fixed', async () => {
    ...
    await writeFile(join(dir, '9998_broken.sql'), 'CREATE TABLE half_done (id int);\nSELECT * FROM no_such_table;\n');
    ...
    expect(applied.map((r) => r.name)).toEqual(SHIPPED);
    ...
    expect(resumed.applied).toEqual(['9998_broken.sql']);
  });
