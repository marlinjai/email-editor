...
describe('health', () => {
  ...
  it('answers 503 when the database does not', async () => {
    ...
    const app = createApp({ sql: dead, dashboardServiceToken: DASHBOARD_TOKEN, log: { error: () => {} } });
...
describe('the error envelope', () => {
  ...
  it('an unexpected failure is internal_error with the request id, and is logged', async () => {
    ...
    const app = createApp({ sql: broken, dashboardServiceToken: DASHBOARD_TOKEN, log: { error: (...a: unknown[]) => logged.push(a) } });
