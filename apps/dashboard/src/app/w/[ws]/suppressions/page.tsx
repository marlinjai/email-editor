...
export default async function SuppressionsPage({ params, searchParams }: { params: Promise<{ ws: string }>; searchParams: Promise<{ q?: string; cursor?: string }> }) {
  ...
  const [list, topics] = await Promise.all([
    act('suppressions.list', async () => (await mail(ws)).api.suppressions.list({ email: q, limit: 50, cursor: sp.cursor })),
