  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        void run(
          () => createWorkspace({ name, slug, companyId }),
          (ws) => router.push(`/w/${ws.id}`),
        );
      }}
    >
      ...
      {companies.length > 1 ? (
        <Field
          id="ws-company"
          label="Company"
          hint="Deleting this company in your Lumitra account deletes the workspace too."
          error={fields.companyId}
        >
        ...
        <p className="text-[13px] text-muted">
          For <span className="text-ink">{companies[0]!.name}</span>. Deleting this company in your Lumitra account deletes the workspace
          too.
        </p>
      ) : (
