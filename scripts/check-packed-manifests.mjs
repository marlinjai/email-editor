#!/usr/bin/env node
/**
 * Pack each published package and check the TARBALL, the artifact a
 ...
 *     sibling packages of the same release set are depended on at that version
 ...
 *
 * Packages are released in sets that share one version and one tag prefix
 * (scripts/release-sets.mjs). Usage:
 *   node scripts/check-packed-manifests.mjs                 check every set, then delete the tarballs
 *   node scripts/check-packed-manifests.mjs --set editor    check one set
 *   node scripts/check-packed-manifests.mjs --set mail --out DIR
 *                                                           keep the checked tarballs in DIR
 *                                                           (the publish workflow uploads exactly these)
 *   node scripts/check-packed-manifests.mjs --set mail --version 0.1.0
...
import { RELEASE_SETS, NOT_RELEASED } from './release-sets.mjs';
...
const setName = argValue('--set');
if (setName !== undefined && !(setName in RELEASE_SETS)) {
  console.error(
    `check-packed-manifests: unknown --set ${JSON.stringify(setName)}; known sets: ${Object.keys(RELEASE_SETS).join(', ')}`
  );
  process.exit(1);
}
if (requiredVersion !== undefined && setName === undefined) {
  console.error(
    'check-packed-manifests: --version needs --set, since each set has its own version.'
  );
  process.exit(1);
}
...
const sets = setName
  ? [[setName, RELEASE_SETS[setName]]]
  : Object.entries(RELEASE_SETS);

// A publishable package that no set lists would never be released or checked.
const listed = new Set(Object.values(RELEASE_SETS).flatMap((set) => set.dirs));
const unlisted = readdirSync(join(repoRoot, 'packages'), {
  withFileTypes: true,
})
  .filter(
    (d) =>
      d.isDirectory() &&
      existsSync(join(repoRoot, 'packages', d.name, 'package.json'))
  )
  .map((d) => ({
    dir: d.name,
    ...readJson(join(repoRoot, 'packages', d.name, 'package.json')),
  }))
  .filter(
    (p) =>
      p.private !== true &&
      !listed.has(p.dir) &&
      !NOT_RELEASED.includes(p.dir)
  );
if (unlisted.length > 0) {
  console.error(
    `check-packed-manifests: ${unlisted.map((p) => p.name).join(', ')} is public but in no release set. Add it to RELEASE_SETS, or to NOT_RELEASED if it is not published yet.`
  );
  process.exit(1);
}
...
for (const [name, set] of sets) {
  const sources = set.dirs.map((dir) => ({
    dir,
    ...readJson(join(repoRoot, 'packages', dir, 'package.json')),
  }));
  const siblingNames = new Set(sources.map((p) => p.name));
  const releaseVersion = requiredVersion ?? sources[0].version;
  console.log(`release set ${name} (${set.tag}${releaseVersion})`);
  ...
  for (const source of sources) {
    ...
    const pkg = JSON.parse(
      execFileSync('tar', ['-xzOf', tarPath, 'package/package.json'], {
        encoding: 'utf8',
      })
    );
    ...
    for (const field of DEP_FIELDS) {
      for (const [dep, range] of Object.entries(pkg[field] ?? {})) {
        if (typeof range === 'string' && range.startsWith('workspace:')) {
          problems.push(
            `${field}.${dep} is still "${range}": the tarball would be uninstallable`
          );
        }
        if (field !== 'devDependencies' && privateNames.has(dep)) {
          problems.push(
            `${field}.${dep} is a private workspace package, which is never published`
          );
        }
        if (
          field !== 'devDependencies' &&
          siblingNames.has(dep) &&
          range !== `^${releaseVersion}`
        ) {
          problems.push(
            `${field}.${dep} is "${range}", expected "^${releaseVersion}" (the version released with it)`
          );
        }
      }
    }
    ...
    if (pkg.version !== releaseVersion)
      problems.push(`version is ${pkg.version}, expected ${releaseVersion}`);
    if (pkg.repository?.url !== REPOSITORY_URL)
      problems.push(
        `repository.url is ${JSON.stringify(pkg.repository?.url)}, expected ${REPOSITORY_URL}`
      );
    if (pkg.repository?.directory !== `packages/${source.dir}`)
      problems.push(`repository.directory is not packages/${source.dir}`);
    ...
    for (const file of new Set(referenced)) {
      if (!files.has(file))
        problems.push(
          `references ${file}, which is not in the tarball (built?)`
        );
    }
    ...
    const leaked = [...files].filter(
      (f) =>
        f.startsWith('src/') ||
        /(^|\/)__tests__\/|\.test\.[cm]?[jt]sx?$/.test(f)
    );
    if (leaked.length > 0)
      problems.push(
        `tarball ships sources or tests: ${leaked.slice(0, 5).join(', ')}${leaked.length > 5 ? ', ...' : ''}`
      );
...
if (checked === 0) {
  console.error(
    'check-packed-manifests: no packages to check. Refusing to pass vacuously.'
  );
  process.exit(1);
}
console.log(
  `\nchecked ${checked} packed package(s): publishable${keepDir ? `, tarballs in ${keepDir}` : ''}`
);
